...
<body>
<form enctype="multipart/form-data" action="<{$app}>/link/del" method="post">
<div class="info_bar"><div class="title_left"></div><div class="title_bg">·&nbsp;友情链接管理</div></div>
<div class="info" style="overflow-y:scroll;">
	<{if $datas}>
      ...
      <tr>
        ...
        <td align="center" bgcolor="#fff"><{if $datas[sn].img}><img src="<{$public}>/uploads/<{$datas[sn].img}>" width='88' height='31'><{else}>暂无<{/if}></td>
        <td align="center" bgcolor="#fff"><{$datas[sn].sort}></td>
        <td align="center" bgcolor="#fff"><a href="<{$app}>/link/mod_index/id/<{$datas[sn].id}>">编辑</a>&nbsp;&nbsp;<a href="<{$app}>/link/del/id/<{$datas[sn].id}>" onClick="return confirm('你确定要删除选中的分类吗?')" >删除</a></td>
      </tr>
      <{/section}>
      </table>
	<{else}>
...
<div class="fun_bar">
	<div class="oper"><input type="button" name="add" class="admin_button" value="新增" onClick="window.location.href='<{$app}>/link/add_index'"/>&nbsp;<input type="submit" name="dels" class="admin_button" value="删除" onClick="return confirm('你确定要删除选中的分类吗?')" /></div>
