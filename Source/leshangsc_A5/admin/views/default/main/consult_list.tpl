...
<body>
<form enctype="multipart/form-data" action="<{$app}>/consult/del" method="post">
<div class="info_bar"><div class="title_left"></div><div class="title_bg">·&nbsp;商品咨询</div></div>
<div class="info" style="overflow-y:scroll;">
	<{if $datas}>
      ...
      <td width="6%" align="center" class="title_color"><span id="all_select" class="all_select">全选</span></td>
        ...
        <td width="20%" align="center" class="title_color">咨询与回复</td>
      ...
      <tr>
        ...
        <td align="center" class="body_color"><span style="color:red">咨询:<{$datas[sn].question}>&nbsp;[<{$datas[sn].q_time|date_format:'%Y-%m-%d %H:%M:%S'}>]</span>&nbsp;<br /><span style="color:green">回复:<{if $datas[sn].answer}><{$datas[sn].answer}><{else}>暂无<{/if}>&nbsp;[<{$datas[sn].a_time|date_format:'%Y-%m-%d %H:%M:%S'}>]</span></td>
        <td align="center" class="body_color"><{$datas[sn].user.user_name}></td>
        <td align="center" class="body_color"><{if $datas[sn].verify}><span style="color:green">已审核</span><{else}><a href="<{$app}>/consult/verify/id/<{$datas[sn].id}>" style="color:red">审核</a><{/if}>&nbsp;&nbsp;<a href="<{$app}>/consult/feedback_index/id/<{$datas[sn].id}>">回复</a>&nbsp;&nbsp;<a href="<{$app}>/consult/del/id/<{$datas[sn].id}>" onClick="return confirm('你确定要删除选中的分类吗?')" >删除</a></td>
      </tr>
      <{/section}>
      </table>
	<{else}>
