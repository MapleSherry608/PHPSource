...
(function( $ ) {
        ...
        diyUpload:function( opt, serverCallBack ) {
			...
			webUploader.on('uploadSuccess',function( file, response ){
				...
				$diyBar.fadeOut( 1000 ,function(){
					$diyBar.text('');
					$diyBar.html('<i class="weui_icon_success"></i>');
					$count=parseInt($(".makeThumbNum").val());
					$(".makeThumbNum").val(++$count);
			...
			webUploader.on('uploadError',function( file, reason ){
				...
				var $diyBar = $fileBox.find('.weui_uploader_status_content');
				$diyBar.text('');
				$diyBar.html('<i class="weui_icon_warn"></i>');
				$diyBar.show();
	...
	function getOption(objId) {
		...
		*/
		return {
			...
			fileNumLimit:50,
			fileSizeLimit:1800 * 1024,
			fileSingleSizeLimit:1800 * 1024
		};
	}
	...
	function createBox( $fileInput, file, webUploader ) {
		var file_id = file.id;
		var $parentFileBox =$('.weui_uploader_bd');
		
		//开始上传,暂停上传,重新上传事件;
		var uploadStart = function (){
			webUploader.upload();
		}
		$('#showTooltips').one('click',uploadStart);
		...
		$parentFileBox.children('.weui_uploader_files').append( li );
		...
		webUploader.makeThumb( file, function( error, dataSrc ) {
			if ( !error ) {	
				...
				var count=parseInt($("#iconCount").text());
				if(++count>9){
					count=9;
					$("#picBtn").remove();
					return;
				...
				$(".makeThumbCount").val(++$count);
				$fileBox.css({'background-image':'url("'+dataSrc+'")'});
			}
		});	
	}
