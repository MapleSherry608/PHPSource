	...
	fileUploader = {
		...
		'init_crawler' : function() {
			...
			this.modalobj.find('#btnFetch').click(function(){
				...
				if (url.length > 0){
					$.post('./admin.php?module=admin&controller=file&action=fetch', {'url':url}, function(data){
		...
		'reset_crawler' : function(){
			...
			this.modalobj.find('#crawler').find('.crawler-img').css("background-image","url('./Public/Admin/Images/nopic.jpg')");
		},
		...
		'browser' : function(path) {
			...
			//初始化数据
			$.getJSON('./index.php?s=/Admin/File/browser.html', {'type':$this.type,'path': path}, function(data){
				if (data['message']){
					alert(data.message);
				} else {
					...
					$browser.find('.btnClose').on('click', function(event){
						...
						if (confirm("确定要删除文件吗？")){
							...
							$.post('./index.php?s=/Admin/File/delete.html', {'file' : attachment}, function(data){
								$($this).parent().remove();
							});
						}
		...
		'init_upload' : function(){
			...
			if(this.modalobj.find('#upload').length == 0){
				...
				var options = {
					...
					paste: '#uploader',
					// swf文件路径
					swf: './Public/Admin/Js/componets/webuploader/Uploader.swf',
					// 文件接收服务端。
					server: './index.php?s=/Admin/File/upload.html',
					chunked: false,
					compress: false,
					formData : {
		...
		'template' : function() {
			...
			template['crawler'] = 
				...
				'					</span>'+
				'				</div>'+
				'				<div class="crawler-img" style="background-image:url(\'./Public/Admin/Images/nopic.jpg\')">'+
