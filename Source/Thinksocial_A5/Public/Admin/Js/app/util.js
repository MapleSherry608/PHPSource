(function(window) {
	...
	util.tomedia = function(src, forcelocal){
		if(src.indexOf('http://') == 0 || src.indexOf('https://') == 0 || src.indexOf('Public/Admin/Js') == 0) {
			return src;
	...
	util.clip = function(elm, str) {
		...
		require(['jquery.zclip'], function(){
			$(elm).zclip({
				path: 'Public/Admin/Js/components/zclip/ZeroClipboard.swf',
				copy: str,
				afterCopy: function(){
	...
	util.iconBrowser = function(callback){
		...
		var modalobj = util.dialog('请选择图标',['./admin.php?s=/Icon/index.html&callback=selectIconComplete'],footer,{containerName:'icon-container'});
		modalobj.modal({'keyboard': false});
