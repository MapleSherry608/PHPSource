<?php
class Zxin_DShop_User
{
	private $sessionid;
	public function __construct()
	{
	    $uniacid = 0;
		$this->sessionid = "__cookie_zxin_shop_201507200000_{$uniacid}";
	}
	function getOpenid()
	{
	...
	function getInfo($base64 = false, $debug = false)
	{
		...
		if (ZXIN_SHOP_DEBUG) {
            $userinfo = array(
                'openid' => 'formUser',
                'nickname' => 'IT~南爵~TI',
                'headimgurl' => 'http://wx.qlogo.cn/mmopen/nbhboGYgz1n6j2V4RC3xj3FwVjvmmW1fV99jUSh1M6LQB7qoaE0GgETapzXzplL6WA8AdC51kEKc67gPC0If2CVbPSwps3cL/0',
                'province' => '浙江',
                'city' => '杭州'
            );
		} else {
		    $openid = session('openid');
			$userinfo_fans = $member_fans->field('membid')->where(array('openid'=>$openid))->find();
			$userinfo = $member->where(array('id'=>$userinfo_fans['membid']))->find();
			$need_openid = true;
	...
	function oauth_info()
	{
		...
		@session_start();
		$sessionid = "__cookie_zxin_shop_201507100000_{$_W['uniacid']}";
