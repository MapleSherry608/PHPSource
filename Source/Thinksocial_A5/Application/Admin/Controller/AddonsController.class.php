...
class AddonsController extends AdminController {
	...
	public function addonlist(){
		$list=M('Addons')->field("id,name,title,description")->select();
...
str;
	...
	public function group(){
		$addname=I('addname');
		$addgroup=M('Menus')->field("id,title,group")->where(array('identify'=>$addname))->select();
	...
	public function groupPost(){
		...
		foreach ($menu as $key => $value) {
			if(!empty($value)&&!empty($key)){
				M('Menus')->save(array('id'=>$key,'group'=>$value));
			}
		}
		$this->redirect('index');
	}
    ...
    public function enable(){
        ...
        S('hooks', null);
		$identify=M('Addons')->where(array('id'=>$id))->getField('name');
		if($identify){
			M('Menus')->where(array('identify'=>$identify))->save(array('status'=>1));
		}
    ...
    public function disable(){
		...
		$identify=M('Addons')->where(array('id'=>$id))->getField('name');
		if($identify){
			M('Menus')->where(array('identify'=>$identify))->save(array('status'=>0));
		}
