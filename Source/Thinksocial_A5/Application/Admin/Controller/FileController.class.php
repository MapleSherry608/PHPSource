...
class FileController extends Controller {
    public function _empty(){
        ...
        if (!empty($option['global']) && empty($isfounder)) {
        ...
        $img_path = ATTACHMENT_ROOT .'/'. $setting['folder'];
        if(!is_dir($img_path)){
        ...
        if(in_array(ACTION_NAME, $action)){
             ...
             * 返回上传数据、并插入数据表
             */
            if (ACTION_NAME == 'upload' || ACTION_NAME == 'fetch') {
                ...
                $img_info = array(
                    'uniacid' => $uniacid,
                    'uid' => $_SESSION['admin_auth']['id'],
                    'filename' => $originname,
                    'attachment' => $pathname,
                    'type' => $type == 'image' ? 1 : 2,
                    'createtime' => TIMESTAMP,
                );
                M('core_attachment')->add($img_info);
                $info = array(
                    'name' => $originname,
                    'ext' => $ext,
                    'filename' => '/Uploads/Picture/'.$pathname,
                    'attachment' => '/Uploads/Picture/'.$pathname,
                    'url' => tomedia('/Uploads/Picture/'.$pathname),
            ...
            if (ACTION_NAME == 'delete') {
                ...
                if (file_delete($attachment)) {
                    if(empty($option['global'])){
                        M('core_attachment')->delete(array('uniacid'=>$uniacid, 'attachment'=>$attachment));
                    }else{
                        M('core_attachment')->delete(array('attachment'=>$attachment));
                    }
                    exit('success');
                } else {
                    exit('failure');
            ...
            if (ACTION_NAME == 'browser') {
                $path = strval($arr['path']);
                $path = str_replace("/Uploads/Picture","", $path);
                ...
                if ($handle = opendir(ATTACHMENT_ROOT.'/'.$dir)) {
                    while (false !== ($filename = readdir($handle))) {
                        ...
                        if (is_dir($file)) {
                            ...
                            $entry = array();
                            $entry['url'] = tomedia('/Uploads/Picture/'.$dir.'/'.$filename);
                            ...
                            $info = array(
                                'level' => $level,
                                'dir' => $dir,
                                'filename' => "/Uploads/Picture/".$entry['filename'],
                                'is_dir' => 0,
                ...
                $result = array(
                    'candelete' => !empty($isfounder) || $role == 'manager',
                    'parent' => $parent,
                    'self' => $self,
                    'crumbs' => $crumbs,
                    'files' => $files
