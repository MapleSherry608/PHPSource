...
class MemberGroupsModel extends Model{
    ...
    protected $_validate = array(
        ...
        array('grade', 'require', '用户组名称不可为空！', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH),
	...
	public function addLevel($data){
		...
		if($this->create($data)){
			$uid = $this->add();
			//action_log('user_add_member', 'Member', $uid, USERID,1);
	...
	public function saveLevel($data){
		if(is_numeric($membid)&&$membid>0){
			if($this->create($data,2)){
				$uid = $this->save();
				//action_log('user_save_member', 'Member', $uid, USERID,1);
				return $uid ? $uid : 0; //0-未知错误，大于0-注册成功
			} else {
				return $this->getError(); //错误详情见自动验证注释
			}
