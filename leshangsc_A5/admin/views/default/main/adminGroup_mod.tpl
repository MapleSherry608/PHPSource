...
<body>
<form enctype="multipart/form-data" action="<{$app}>/adminGroup/mod" method="post">
<div class="info_bar"><div class="title_left"></div><div class="title_bg">·&nbsp;编辑管理员组</div></div>
            ...
            <{section name=sn loop=$class}>
              ...
              <tr>
              	 <{section name=n loop=$node}>
                    <{if $class[sn].id==$node[n].class_id}><td><input type="checkbox" name="auth[]" value="<{$node[n].id}>"
                     <{section name=a loop=$datas.auth}>
                     <{if $node[n].id==$datas.auth[a]}>checked="checked" <{/if}>
                     <{/section}>
                    />&nbsp;<{$node[n].name}></td><{/if}>
                 <{/section}>
              </tr>
