...
class WeiXinAccount extends WeAccount {
	...
	public function __construct($acid = array()) {
		parent::__construct();
		if (empty($acid)) {
			return true;
		}
		$this->acid=$acid;
		$this->account = uni_accounts($acid);
	...
	public function menuQuery() {
		...
		$content = ihttp_get($url);
		if(!is_array($content)) {
			$this->error('接口调用失败，请重试！' . (is_string($content) ? "微信公众平台返回元数据: {$content}" : ''));
		}
		$dat = $content['content'];
		$result = json_decode($dat, true);
	...
	public function fansAll($nextOpenid=null) {
		$token = $this->fetch_token();
		if(is_error($token)){
			return $token;
		}
	...
	public function fetch_token() {
		if(!empty($this->account['access_token'])
			...
			&& $this->account['access_token']['expire'] > time()&& FALSE) {
		...
		$content = ihttp_get($url);
		if(is_error($content)) {
			$this->error('获取微信公众号授权失败, 请稍后重试！错误详情: ' . $content['message']);
		}
		$token = @json_decode($content['content'], true);
		if(empty($token) || !is_array($token) || empty($token['access_token']) || empty($token['expires_in'])) {
			$errorinfo = substr($content['meta'], strpos($content['meta'], '{'));
			$errorinfo = @json_decode($errorinfo, true);
			$this->error('获取微信公众号授权失败, 请稍后重试！ 公众平台返回原始数据为: 错误代码-' . $errorinfo['errcode'] . '，错误信息-' . $errorinfo['errmsg']);
		}
		$record = array();
		$record['token'] = $token['access_token'];
		$record['expire'] = time() + $token['expires_in'] - 300;
		$row = array();
		$row['access_token'] = iserializer($record);
		M('MemberPublic')->where(array('id' => $this->acid))->save($row);
	...
	public function fetch_available_token() {
		...
		foreach ($accounts as $account) {
			...
			if (!empty($account['access_token'])) {
				...
				if ($account['access_token']['expire'] > time()) {
	...
	public function getJsApiTicket(){
		...
		&& $this->account['jsapi_ticket']['expire'] > time()) {
