...
class Zxin_DShop_Excel
{
	...
	function export($list, $params = array())
	{
		...
		require_once IA_ROOT . '/Api/phpexcel/PHPExcel.php';
		$excel = new PHPExcel();
		$excel->getProperties()->setCreator("搜雪商城")->setLastModifiedBy("搜雪商城")->setTitle("Office 2007 XLSX Test Document")->setSubject("Office 2007 XLSX Test Document")->setDescription("Test document for Office 2007 XLSX, generated using PHP classes.")->setKeywords("office 2007 openxml php")->setCategory("report file");
	...
	public function import($excefile)
	{
		global $_W;
		require_once IA_ROOT . '/Api/phpexcel/PHPExcel.php';
		require_once IA_ROOT . '/Api/phpexcel/PHPExcel/IOFactory.php';
		require_once IA_ROOT . '/Api/phpexcel/PHPExcel/Reader/Excel5.php';
		$path = IA_ROOT . "/data/virtual/tmp/";
		if (!is_dir($path)) {
			load()->func('file');
			mkdirs($path, '0777');
		}
		...
		if (empty($tmpname)) {
			$this->error('请选择要上传的Excel文件!');
		}
		$ext = strtolower(pathinfo($filename, PATHINFO_EXTENSION));
		if ($ext != 'xlsx') {
			$this->error('请上传 xlsx 格式的Excel文件!');
		}
		$uploadfile = $path . $file;
		$result = move_uploaded_file($tmpname, $uploadfile);
		if (!$result) {
		    $this->error('上传Excel 文件失败, 请重新上传!');
		}
