...
class AuthManagerController extends AdminController{
    ...
    public function changeStatus($method=null){
        ...
        switch ( strtolower($method) ){
            ...
            case 'deletegroup':
			        ...
			        if( M("AuthGroup")->where($where)->delete()!==false ) {
    ...
    public function user($group_id){
        ...
        $_REQUEST = array();
        $list = $this->lists($model,array('a.group_id'=>$group_id,'m.status'=>array('egt',0)),'m.id asc',null,'m.uid,m.username,m.last_login_time,m.last_login_ip,m.status');
        int_to_string($list);
        $this->assign( '_list',     $list );
        $this->assign('auth_group', $auth_group);
        $this->assign('this_group', $auth_group[(int)$_GET['group_id']]);
	...
	public function addGroupUser(){
		$group_id=I('group_id');
		empty($group_id) && $this->error('非法访问');
		$prefix   = C('DB_PREFIX');
		$total =   M()->query("SELECT COUNT(*) as 'countname' FROM `".$prefix."user` sm WHERE id NOT IN (SELECT uid FROM `".$prefix."auth_group_access` sa  WHERE sa.`group_id`=".$group_id.")");
		$total=$total[0]['countname'];
		$REQUEST    =   (array)I('request.');
		$page = new \Think\Page($total, 10, $REQUEST);
		$p =$page->show();
        $this->assign('_page', $p? $p: '');
        $this->assign('_total',$total);
        $userlist    = M()->query("SELECT id,username,status FROM `".$prefix."user` sm WHERE id NOT IN (SELECT uid FROM `".$prefix."auth_group_access` sa  WHERE sa.`group_id`=".$group_id.") limit ".$page->firstRow.",".$page->listRows);
